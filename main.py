...
def get_macd_signal():
    ...
    for attempt in range(max_retries):
        try:
            ...
            klines = client.get_klines(symbol=TRADE_SYMBOL, interval=Client.KLINE_INTERVAL_1HOUR, limit=100, startTime=start_time)
...
async def macd_signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        response = [
            f"<b>{TRADE_SYMBOL} @ {current_price:.4f} (1h)</b>",
...
async def get_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for attempt in range(max_retries):
        try:
            ...
            eth_balance_info = next((asset for asset in balance_info['balances'] if asset['asset'] == "ETH"), None)
            usdc_balance_info = next((asset for asset in balance_info['balances'] if asset['asset'] == "USDC"), None)
            
            eth_free = float(eth_balance_info['free']) if eth_balance_info else 0.0
            usdc_free = float(usdc_balance_info['free']) if usdc_balance_info else 0.0
            
            logging.info(f"Balance: ETH={eth_free:.6f}, USDC={usdc_free:.2f}")
...
async def get_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logging.info(f"Price: {price:.4f} USDC")
            await update.message.reply_text(f"📈 Поточна ціна {TRADE_SYMBOL}: {price:.4f} USDC")
            return
        except Exception as e:
...
async def show_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for trade in reversed(trade_history[-10:]): 
        ...
        messages.append(f"{trade['date']} - {trade['type']} {trade['amount']:.6f} ETH за {trade['price']:.4f} USDC (Зміна балансу: {profit_loss_color})") 
...
async def toggle_auto_trading(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if auto_trading_enabled:
        delay_to_first_execution = get_seconds_to_next_20_minute()

        logging.info(f"Scheduling auto trading to start after {delay_to_first_execution} seconds (at the next XX:20) with interval {AUTO_TRADE_INTERVAL} seconds")
        ...
        await update.message.reply_text(f"✅ Автотрейдинг увімкнено!\nПерша перевірка через {int(delay_to_first_execution / 60)} хвилин (о XX:20). Наступні перевірки щогодини о XX:20.")
    else:
        ...
        try:
            if side == "BUY":
                ...
                if current_price <= 0:
                    ...
                    return "Помилка: Поточна ціна ETH є нульовою або від'ємною."

                quantity_eth_raw = amount_to_spend / current_price

                rounding_precision = Decimal('1E-%d' % qty_precision)

                quantity_eth_decimal = (quantity_eth_raw / step_size).quantize(Decimal('1'), rounding=ROUND_DOWN) * step_size
                quantity_decimal = quantity_eth_decimal.quantize(rounding_precision, rounding=ROUND_DOWN)

                if quantity_eth_decimal < min_qty:
                    calculated_min_notional = min_qty * current_price
                    if calculated_min_notional < min_notional:
                        return f"⚠️ Розрахована кількість ({quantity_eth_decimal:.{qty_precision}f} ETH) замала навіть для мінімального ордеру ({min_qty:.{qty_precision}f} ETH * {current_price:.2f} = {calculated_min_notional:.2f} USDC), що менше {min_notional:.2f} USDC"
                    return f"⚠️ Розрахована кількість {quantity_eth_decimal:.{qty_precision}f} ETH нижче мінімуму {min_qty}, і навіть мінімальний ордер ({min_qty:.{qty_precision}f} ETH * {current_price:.2f} = {calculated_min_notional:.2f} USDC) менший за {min_notional:.2f} USDC"
                    quantity_eth_decimal = min_qty

                if quantity_eth_decimal > max_qty:
                    return f"⚠️ Розрахована кількість {quantity_eth_decimal:.4f} перевищує максимум {max_qty}"
                ...
                return f"🟢 Купівля: {trade_data['amount']:.6f} ETH за {trade_data['price']:.4f} USDC"

            elif side == "SELL":
                balance_info = client.get_account()
                eth_balance_info = next((asset for asset in balance_info['balances'] if asset['asset'] == "ETH"), None)
                eth_balance = Decimal(eth_balance_info['free']) if eth_balance_info else Decimal('0')
                logging.info(f"ETH balance for SELL: {eth_balance}")

                if eth_balance < min_qty:
                    logging.warning(f"ETH balance {eth_balance} is less than minQty {min_qty} for SELL.")
                    return f"⚠️ Недостатньо ETH для продажу. Мінімальна кількість: {min_qty:.6f}. У вас: {eth_balance:.6f}"

                rounding_precision = Decimal('1E-%d' % qty_precision)

                quantity_eth_decimal = (eth_balance / step_size).quantize(Decimal('1'), rounding=ROUND_DOWN) * step_size
                quantity_decimal = quantity_eth_decimal.quantize(rounding_precision, rounding=ROUND_DOWN)

                if quantity_eth_decimal > max_qty:
                    return f"⚠️ Розрахована кількість {quantity_eth_decimal:.2f} перевищує максимум {max_qty}"
                ...
                return f"🔴 Продаж: {trade_data['amount']:.6f} ETH за {trade_data['price']:.4f} USDC"
...
async def buy_eth_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info("Buy ETH command triggered")
    await update.message.reply_text("Спроба купівлі ETH...")
    result = execute_market_trade("BUY")
    await update.message.reply_text(result)

async def sell_eth_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logging.info("Sell ETH command triggered")
    await update.message.reply_text("Спроба продажу ETH...")
...
async def check_macd_and_trade(context: ContextTypes.DEFAULT_TYPE):
    ...
    if signal_action == "BUY":
        logging.info("MACD BUY signal detected (green histogram)!")
        trade_message = execute_market_trade("BUY")
    elif signal_action == "SELL":
        logging.info("MACD SELL signal detected (red histogram)!")
    ...
    if trade_message:
        ...
        response_parts = [
            f"<b>Автотрейдинг ({datetime.now().strftime('%H:%M')}):</b>",
            f"<b>{TRADE_SYMBOL} @ {current_price:.4f} (1h)</b>",
            f"<b>MACD: {hist_color_emoji} {result['histogram'][-1]:.2f}</b>",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trade_keyboard = [
        ["💰 Перевірити баланс", "📈 Ціна ETH"],
        ["📊 MACD сигнал", "🤖 Автотрейдинг"],
        ["🟢 Купити ETH", "🔴 Продати ETH"],
        ["📊 Статистика"]
    ]
    reply_markup = ReplyKeyboardMarkup(trade_keyboard, resize_keyboard=True)
    await update.message.reply_text(
        "🔷 Торгівельний бот Binance\n\n"
        "🤖 Автотрейдинг - автоматичні угоди за сигналами MACD\n"
        "📊 MACD сигнал - перевірка поточного стану\n"
        "🟢 Купити ETH - купівля ETH за всю суму USDC за ринковою ціною\n"
        "🔴 Продати ETH - продаж усього ETH за ринковою ціною\n"
...
def main():
    ...
    application.add_handler(MessageHandler(filters.Regex("^(📈 Ціна ETH)$"), get_price))
    ...
    application.add_handler(MessageHandler(filters.Regex("^(🟢 Купити ETH)$"), buy_eth_command))
    application.add_handler(MessageHandler(filters.Regex("^(🔴 Продати ETH)$"), sell_eth_command))
